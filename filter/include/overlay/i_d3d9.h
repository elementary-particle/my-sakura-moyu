#ifndef FILTER_I_D3D9_H
#define FILTER_I_D3D9_H
...
extern PFUNC_Direct3DCreate9 pDirect3DCreate9;
...
class IOverlayDevice : public IDirect3DDevice9 {
 private:
  struct VERTEX {
    ...
    UINT32 Color;
    FLOAT TU, TV;
    VERTEX(INT X_, INT Y_, INT Z_, FLOAT TU_, FLOAT TV_)
        : X((FLOAT)X_ - 0.5f),
          Y((FLOAT)Y_ - 0.5f),
          Z((FLOAT)Z_),
          RHW(1.0f),
  ...
  static const DWORD dwOverlayFVF =
      D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1;
  HANDLE hFrameCountThread;
  IOverlayD3D *iOverlayD3D;
  IDirect3DDevice9 *iDevice;
  IDirect3DStateBlock9 *iOverlayState;

 public:
  struct FRAMEDATA {
    IOverlayDevice *iDevice;
    HANDLE hThread;
    HANDLE hRasterEvent, hRenderEvent;
    std::queue<IDirect3DTexture9 *> TextureList;
    std::queue<DWORD> ColorList;
    IDirect3DVertexBuffer9 *iVertexBuffer;
    LONGLONG llInterval;
    FRAMEDATA() {
      hRasterEvent = CreateEventW(NULL, FALSE, FALSE, NULL);
      hRenderEvent = CreateEventW(NULL, FALSE, FALSE, NULL);
      iVertexBuffer = NULL;
      llInterval = CHAIN_SIZE * 1000 / 60;
    }
    ~FRAMEDATA() {
      CloseHandle(hRenderEvent);
      CloseHandle(hRasterEvent);
    }
  };
  ...
  LONG RenderIndex, bRunCount, uCount, uFrameRate;
  UINT bRunRender;
  UINT uViewWidth, uViewHeight;
  CRITICAL_SECTION SubSection, RenderSection;
  Subtitle *pSub;
  LONGLONG llTickStart;
  HRESULT SoftBlend(FRAMEDATA *pFrame);
  HRESULT RasterFrame(FRAMEDATA *pFrame);
  HRESULT RenderFrame(FRAMEDATA *pFrame);
  static void ReleaseFrame(FRAMEDATA *pFrame);
  void AttachSub(Subtitle *pSub_);
  void DetachSub();
  HRESULT CreateDeviceResources(D3DPRESENT_PARAMETERS *pPresentationParameters);
  HRESULT ReleaseDeviceResources();

  IOverlayDevice(IOverlayD3D *_iOverlayD3D, IDirect3DDevice9 *_iDevice,
                 D3DPRESENT_PARAMETERS *pPresentationParameters, BOOL bSoft);
  ~IOverlayDevice();
  ...
  STDMETHOD(GetCurrentTexturePalette)(THIS_ UINT *pPaletteNumber);
  ...
  (THIS_ D3DPRIMITIVETYPE, INT BaseVertexIndex, UINT MinVertexIndex,
...
class IOverlayD3D : public IDirect3D9 {
 ...
 public:
  ...
  (THIS_ UINT Adapter, DWORD Flags, D3DADAPTER_IDENTIFIER9 *pIdentifier);
  STDMETHOD_(UINT, GetAdapterModeCount)(THIS_ UINT Adapter, D3DFORMAT Format);
  STDMETHOD(EnumAdapterModes)
  (THIS_ UINT Adapter, D3DFORMAT Format, UINT Mode, D3DDISPLAYMODE *pMode);
  STDMETHOD(GetAdapterDisplayMode)(THIS_ UINT Adapter, D3DDISPLAYMODE *pMode);
  STDMETHOD(CheckDeviceType)
  (THIS_ UINT Adapter, D3DDEVTYPE DevType, D3DFORMAT AdapterFormat,
   D3DFORMAT BackBufferFormat, BOOL bWindowed);
  STDMETHOD(CheckDeviceFormat)
  (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT AdapterFormat,
  ...
  (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT SourceFormat,
   D3DFORMAT TargetFormat);
  STDMETHOD(GetDeviceCaps)
  (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, D3DCAPS9 *pCaps);
